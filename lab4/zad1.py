    ...
    for sock in read_sockets:
        if sock == server_socket:
            ...
            try:
                data = sock.recv(4096)
                if data:
                    timestamp = strftime("%Y-%m-%d %H:%M:%S", gmtime()).encode()
                    sock.send(timestamp)
                    print("[%s] Echoed data to client %s" % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), sock.getpeername()))
            except:
                print("[%s] Error occurred with client %s" % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), sock.getpeername()))
                sock.close()
